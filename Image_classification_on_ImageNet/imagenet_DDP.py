...
import math
from disributedrandomclasssample import DistributedSamplerWrapper
from samplers import RandomClassSampler


parser = argparse.ArgumentParser(description='PyTorch ImageNet Training')
parser.add_argument('-data', default='/data/moy20/Imagenet/Image',type=str,
...
parser.add_argument("-style_position",type=int,default=0,help="where to add the style")
...
def main():
    ...
    print("每个节点GPU数量：",ngpus_per_node)
    if args.multiprocessing_distributed:
        # Since we have ngpus_per_node processes per node, the total world_size
        # needs to be adjusted accordingly
        args.world_size = ngpus_per_node * args.world_size # 相当于总进程数量
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if 'res' in args.model:
        model = eval('networks.resnet.' + args.model)(add_style=args.add_style,styleratio1=args.styleratio1,styleratio2=args.styleratio2,style_type=args.style_type,style_position=args.style_position,seed=args.seed)
    elif 'dense' in args.model:
        model = eval('networks.densenet.' + args.model)(add_style=args.add_style,styleratio1=args.styleratio1,styleratio2=args.styleratio2,style_type=args.style_type,style_position=args.style_position,seed=args.seed)
    else:
    ...
    if args.distributed:
        # mysampler = RandomClassSampler(train_dataset,args.batch_size,6)
        # train_sampler = DistributedSamplerWrapper(mysampler)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if not args.multiprocessing_distributed or (args.multiprocessing_distributed
            ...
            np.savetxt("./"+args.model+"accuracy.txt", np.array(acc_list))
            save_checkpoint({
                'epoch': epoch + 1,
