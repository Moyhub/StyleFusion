...
class DenseNet(nn.Module):
    ...
    def __init__(self, growth_rate=12, block_config=(16, 16, 16), compression=0.5,
                 num_init_features=24, bn_size=4, drop_rate=0, num_classes=1000,
    ...
    def forward(self, x, label=None):
        ...
        features = F.avg_pool2d(out, kernel_size=self.avgpool_size).view(features.size(0), -1)
...
def densenet121(add_style=False, styleratio1=0,styleratio2=0, style_type=["Inclass"],style_position=[0],seed=818):
    model = DenseNet(growth_rate=32, block_config=(6, 12, 24, 16), small_inputs=False, add_style=add_style,styleratio1=styleratio1,styleratio2=styleratio2,style_type=style_type,style_position=style_position,seed=seed)
    return model


def densenet169(**kwargs):
    model = DenseNet(growth_rate=32, block_config=(6, 12, 32, 32), small_inputs=False, )
    return model


def densenet201(**kwargs):
    model = DenseNet(growth_rate=32, block_config=(6, 12, 48, 32), small_inputs=False, )
    return model


def densenet265(add_style=False, styleratio1=0,styleratio2=0, style_type=["Inclass"],style_position=[0],seed=818):
    model = DenseNet(growth_rate=32, block_config=(6, 12, 64, 48), small_inputs=False, add_style=add_style,styleratio1=styleratio1,styleratio2=styleratio2,style_type=style_type,style_position=style_position,seed=seed)
    return model