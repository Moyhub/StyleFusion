...
class ResNet_Cifar(nn.Module):
    ...
    def __init__(self, block, layers, num_classes = 10,add_style=False, styleratio1=0,styleratio2=0, style_type=["Inclass","Outclass"],style_position=[3],dropout_rate=0,seed=169):
    ...
    def forward(self, x,label=None):
        ...
        x = self.avgpool(x)
        x = x.view(x.size(0), -1) # 128*64*1*1
        x = self.fc(x)
